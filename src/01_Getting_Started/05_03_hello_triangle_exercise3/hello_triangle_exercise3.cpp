...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Ejercicios - Capítulo 5", nullptr, nullptr);
    if (!window)
    {
    ...
    if (!success)
    {
        glGetShaderInfoLog(fragmentShader2, 255, nullptr, infoLog);
        std::cout << "ERROR::SHADER::FRAGMENT::COMPILATION_FAILED\n" << infoLog << std::endl;
    }
    // Link shaders
    ...
    if (!success)
    {
        glGetProgramInfoLog(shaderProgram1, 255, nullptr, infoLog);
    ...
    if (!success)
    {
        glGetProgramInfoLog(shaderProgram2, 255, nullptr, infoLog);
        std::cout << "ERROR::SHADER::PROGRAM::LINKING_FAILED\n" << infoLog << std::endl;
    }
